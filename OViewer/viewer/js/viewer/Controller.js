define([
...
], function (Map, dom, domStyle, domGeom, domClass, on, array, BorderContainer, ContentPane, FloatingTitlePane, lang, mapOverlay, FloatingWidgetDialog, put, aspect, has, topic, PopupMobile, Menu,TabContainer) {
	...
	return {
		...
		//设置panes: center, left 和outer,放置容器，在sidebar上放contentPane
		initPanes: function () {
...
//放置总容器-border container
	     ...
	     for(tab in this.tabs){
				 tabOptions = lang.clone(this.tabs[tab]);
				  this.tabs[tab]=new ContentPane(tabOptions).placeAt(tabContainer);
			}
...
////// initPanes结束
		...
		_createTitlePaneWidget: function (parentId,group, title, position, open, canFloat, placeAt) {
			var tp, options = {
					title: title || 'Widget',
					open: open || false,
			...
			//	placeAt = this.panes.left;
				if(group==1)
			  	placeAt = this.tabs.tab1;
				if (group==2)
				placeAt = this.tabs.tab2;
			if (group==3)
					placeAt = this.tabs.tab3;
			}
			if (!placeAt) {
			} else {
				...
				tp = new FloatingTitlePane(options).placeAt(placeAt);
				tp.startup();
			}
			return tp;
		},
		...
		_createContentPaneWidget: function (parentId,group, title, className, region, placeAt) {
			var cp, options = {
					title: title,
		...
		widgetLoader: function (widgetConfig, position) {
			...
			if ((widgetConfig.type === 'titlePane' || widgetConfig.type === 'contentPane' || widgetConfig.type === 'floating') && (widgetConfig.id && widgetConfig.id.length > 0)) {
				...
				if (widgetConfig.type === 'titlePane') {
					pnl = this._createTitlePaneWidget(parentId,widgetConfig.group, widgetConfig.title, position, widgetConfig.open, widgetConfig.canFloat, widgetConfig.placeAt);
				} else if (widgetConfig.type === 'contentPane') {
					pnl = this._createContentPaneWidget(parentId,widgetConfig.group, widgetConfig.title, widgetConfig.className, widgetConfig.region, widgetConfig.placeAt);
