define([
    ...
    'xstyle/css!./Identify/css/Identify.css'
             ...
             array, all, topic, query, domStyle, domClass,domConstruct,dojoColor,
             Moveable, Memory, ContentPane,TabContainer,IdentifyTask,
             IdentifyParameters, QueryTask,PopupTemplate, SimpleMarkerSymbol,IdentifyTemplate, i18n) {
    return declare([_WidgetBase, _TemplatedMixin, _WidgetsInTemplateMixin], {
        ...
        //查询井室及附件属性
        queryVcAndAff:function(feature){
            var compFeatures=[];//  补偿器对象
            var exhFeatures=[]; //排气泄水对象
            var imgPath="http://10.50.51.67/hrb/vcImages/";
           var exhQueryTask=new QueryTask("http://10.50.51.67:6080/arcgis/rest/services/test/hrbPipe_2016_3/MapServer/17");
           var exhQuery=new Query();
            //补偿器
          var  compQueryTask=new QueryTask("http://10.50.51.67:6080/arcgis/rest/services/test/hrbPipe_2016_3/MapServer/18");
          var  compQuery=new Query();
            compQuery.returnGeometry=false;
            exhQuery.returnGeometry=false;
            compQuery.outFields=["*"];
            exhQuery.outFields=["*"];
            compQuery.where=" ";
            ...
            symbol.setSize(15);
            symbol.setColor(new dojoColor([0,191,255,0.5]))
            graphic.setSymbol(symbol);
            this.map.graphics.clear();
            this.map.graphics.add(graphic);

            var vcCP = new ContentPane({
                title: "井室属性",
                content: "test1"
            });
            //总TabContainer
            var pContainer = new TabContainer({style: "height:280px;",
                title:"井室"
            }, domConstruct.create("div"));
            //井室Container
            var vcContainer= new TabContainer({
                title:"井室"
            }, domConstruct.create("div"));

            var compCp=new ContentPane({
                title: "补偿器"
            });
            //井室contentPane

            var subDevices= new TabContainer({
                title:"附属设备"
            }, domConstruct.create("div"));
            vcCP.setContent(getValveChamberAttr(feature));
            var imgContent="<p><img id='sarah' src='"+imgPath+imgUrl+".jpg' style='width:200px; height:150px'/></p>";
            if(vcCode.length>0) {
                compQuery.where = "ValveChamb='VCH00" + vcCode.substr(3) + "'";
                exhQuery.where = "VCID='" + vcCode + "'";
                var exhDevices = exhQueryTask.execute(exhQuery);
                var compDevices = compQueryTask.execute(compQuery);
                var promises = all([exhDevices, compDevices]);
                var imagePane = new ContentPane({
                    title: "图片",
                    content: imgContent,
                    onClick: function(){
                        window.open(imgPath+imgUrl+".jpg");
                    }
                });
                ...
                imagePane.setContent(imgContent);

                vcContainer.addChild(imagePane);

                pContainer.addChild(vcContainer);
                template.setContent(tc.domNode) ;
            ...
            function handleQueryResults(results)  {
                ...
                exhFeatures.length=0;
                // Display attribute information.
                if(results[1].features.length>0)
                {
                    array.forEach(results[1].features, function(feat) {
                        var cp = new ContentPane({
                            title: "补偿器"
                        });
                        cp.setContent(getCompContent(feat));
                        compFeatures.push(cp);
                    });
                }
                ...
                if(results[0].features.length>0){
                    array.forEach(results[0].features, function(feat) {
                        ...
                        cp.setContent(getExhContent(feat));
                        exhFeatures.push(cp);
                    });
                }

                vcContainer.addChild(vcCP);

                imagePane.setContent(imgContent);

                vcContainer.addChild(imagePane);

                array.forEach(compFeatures,function(comp){
                    subDevices.addChild(comp);
                });
                array.forEach(exhFeatures,function(exhDevice){
                    subDevices.addChild(exhDevice);
                });
                pContainer.addChild(vcContainer);
                //既没有补偿器也没有排气泄水
                if(results[0].features.length>0||results[1].features.length>0)
                    pContainer.addChild(subDevices);
                template.setContent(pContainer.domNode) ;
                feature.setInfoTemplate(template);
                map.infoWindow.show(event.mapPoint);
                map.infoWindow.setFeatures([deferred]);
                //      handle.hide();
